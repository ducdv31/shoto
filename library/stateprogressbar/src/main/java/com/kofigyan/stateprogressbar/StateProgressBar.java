...
public class StateProgressBar extends View {
    ...
    public enum StateHighlightMode {
        ...
        static StateHighlightMode fromInt(int value) {
            switch (value) {
                case 0:
                    return StateHighlightMode.NONE;
                case 1:
                    return StateHighlightMode.CURRENT;
                case 2:
                    return StateHighlightMode.FINISHED;
            }
            return StateHighlightMode.CURRENT;
        }
    }
    ...
    private void initStateProgressBar(Context context) {
        ...
        mCurrentBackgroundColor = ContextCompat.getColor(context, R.color.red);
    ...
    private void drawBackgroundCircles(Canvas canvas) {
        int startIndex;
        if (mCurrentStateNumber == 0) {
            startIndex = 0;
        } else
            startIndex = mIsStateNumberDescending ? 0 : mCurrentStateNumber;
    ...
    private void drawBackgroundCurrent(Canvas canvas) {
        if (mCurrentStateNumber != 0) {
            int startIndex = mCurrentStateNumber - 1;
            int endIndex = mCurrentStateNumber;
            Paint colorP = setPaintAttributes(mStateLineThickness, mCurrentBackgroundColor);
            colorP.setColor(mCurrentBackgroundColor);
            drawCircles(canvas, colorP, startIndex, endIndex);
        }
    }
    ...
    private void drawForegroundCircles(Canvas canvas) {
        int startIndex;
        if (mCurrentStateNumber == 0) {
            startIndex = mMaxStateNumber - 1;
        } else
            startIndex = mIsStateNumberDescending ? mMaxStateNumber - mCurrentStateNumber : 0;
    ...
    private void drawBackgroundLines(Canvas canvas) {
        ...
        if (mCurrentStateNumber == 0) {
            ...
            startIndex = mIsStateNumberDescending ? 0 : mCurrentStateNumber - 1;
    ...
    private void drawForegroundLines(Canvas canvas) {
        int startIndex;
        if (mCurrentStateNumber == 0) {
            startIndex = mMaxStateNumber - mCurrentStateNumber;
        } else
            startIndex = mIsStateNumberDescending ? mMaxStateNumber - mCurrentStateNumber + 1 : 0;
    ...
    private boolean isStateHighlighted(int state) {
        switch (mStateHighlightMode) {
            case NONE:
                return false;
            case CURRENT:
                return state == mCurrentStateNumber;
            case FINISHED:
                return state <= mCurrentStateNumber;
        }
        // unreachable
